...
var   dz   := 5.0   ## distancia en Z de la cámara al origen
...
func _actualiza_transf_vista(  ) -> void : 
	var ahr  := ((6.0+float(dxy.x))*2.0*PI)/360.0 ## ang.horiz.radi.
	var avr  := ((15.0+float(dxy.y))*2.0*PI)/360.0 ## ang.vert.radi.
	var tras := Transform3D().translated( Vector3( 0.0, 2.0, dz))	
