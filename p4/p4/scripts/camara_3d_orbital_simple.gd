...
var   dz   := 22.0   ## distancia en Z de la cámara al origen
var   dxy  := Vector2( 0.0, -15.0 ) ## ángulos hor. y vert. (en grados)

## -------------------------------------------------------------------
## actualiza la variable 'transform' de este nodo cámara

func _actualiza_transf_vista(  ) -> void : 
	var ahr  := ((0.0+float(dxy.x))*2.0*PI)/360.0 ## ang.horiz.radi.
	var avr  := ((30.0+float(dxy.y))*2.0*PI)/360.0 ## ang.vert.radi.
	var tras := Transform3D().translated( Vector3( 0.0, 4.0, dz))	
